  ...
  return (
    <Link href={`/quiz/${quiz.id}`} className="w-full">
      <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        className="relative overflow-hidden rounded-xl border-2 border-purple-700/20 bg-[#1A1F2B]/90 p-6 backdrop-blur-sm hover:border-purple-600/30 transition-all duration-200 cursor-pointer h-full flex flex-col justify-between"
      >
        <div className="absolute inset-0 bg-gradient-to-b from-purple-900/20 to-transparent opacity-50"></div>
        <div className="relative flex-grow">
          <h3 className="text-lg font-semibold text-white mb-2 line-clamp-2">{quiz.title}</h3>
        </div>
        <p className="text-sm text-gray-400 mt-2">{quiz.questions ? quiz.questions.length : 0} Questions</p>
      </motion.div>
    </Link>
  )
}
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-start justify-center pt-16">
      <div className="w-full max-w-7xl mx-auto p-4 lg:p-8">
        <h1 className="text-2xl lg:text-3xl font-semibold mb-6 text-white text-center">Generated Quizzes</h1>
        ...
        <div className="flex justify-center w-full">
          {isLoading ? (
            ...
            <div className="text-center text-red-400 p-4 rounded-lg bg-red-900/20 w-full">{error}</div>
          ) : quizzes.length === 0 ? (
